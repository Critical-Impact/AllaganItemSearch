...
public abstract class ItemRelicWeaponSourceRenderer<T> : ItemInfoRenderer<T> where T : ItemRelicWeaponSource
{
    ...
    public ItemRelicWeaponSourceRenderer(ITextureProvider textureProvider, IDalamudPluginInterface dalamudPluginInterface, ItemSheet itemSheet, MapSheet mapSheet, ILocalizer<RelicWeaponType> relicWeaponTypeLocalizer)
    {
        this.TextureProvider = textureProvider;
        this.ItemSheet = itemSheet;
        this._relicWeaponTypeLocalizer = relicWeaponTypeLocalizer;
    }
            ...
            using (var table = ImRaii.Table("forms", items.Any(c => c.OffhandItemId != 0) ? 3 : 2))
            {
                if (table)
                {
                    foreach (var relicWeapon in items)
                    {
                        ...
                        ImGui.TextUnformatted($"{this._relicWeaponTypeLocalizer.Format(relicWeapon.Type)}");
                        ImGui.TableNextColumn();
                        var item = this.ItemSheet.GetRow(relicWeapon.ItemId);
                        ImGui.Image(
                        ...
                        {
                            ImGui.TableNextColumn();
                            item = this.ItemSheet.GetRow(relicWeapon.OffhandItemId);
                            ImGui.Image(
                                this.TextureProvider
                                    .GetFromGameIcon(new GameIconLookup(item.Icon))
                                    .GetWrapOrEmpty().Handle,
    ...
    public override Func<ItemSource, int> GetIcon => _ => AllaganLib.Shared.Misc.Icons.WeaponIcon;
}
